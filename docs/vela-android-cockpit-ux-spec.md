...
| Paper order preparation — guided, read only | Paper Trading | A single `Preparar orden Paper` action coordinates preflight → draft → preview → readiness and stops before arm, token, confirmation, or POST. The former step-by-step action buttons are not exposed in the production Paper section. |
| Paper execution readiness — disabled | Paper Trading | Readiness is reported inside the automatic read-only preparation summary. `executionEnabled=false`, `REAL locked=true`, Paper POST/LIVE disabled, and `Auto Paper=false` remain fail-closed invariants. |
