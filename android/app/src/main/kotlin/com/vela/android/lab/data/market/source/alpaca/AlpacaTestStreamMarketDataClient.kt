...
/**
 * Read-only [MarketDataClient] for the Alpaca Market Data **test
 * stream** at `wss://stream.data.alpaca.markets/v2/test`.
 *
 * Phase 2.b safety contract:
 *
 *  - The constructor calls [AlpacaStreamEndpoint.requireSafeReadOnlyEndpoint]
 *    on the endpoint. Any URL except the hard-coded test stream is
 *    rejected at construction time.
 *  - This class **has no method that submits an order**, mutates
 *    account state, or performs any trading action. The Phase 2.a
 *    reflection-based contract tests cover both this class and the
 *    `MarketDataClient` interface to prove it.
 ...
 *
 * The WebSocket transport is injected via [AlpacaWebSocketFactory]
 * so tests can drive the client with an in-memory fake.
 */
class AlpacaTestStreamMarketDataClient(
    ...
    private val endpoint: String = AlpacaStreamEndpoint.TEST_STREAM_URL,
) : MarketDataClient {

    init {
        AlpacaStreamEndpoint.requireSafeReadOnlyEndpoint(endpoint)
    }

    override val source: MarketDataSource = MarketDataSource.ALPACA_TEST_STREAM
    ...
    private val healthTracker: StreamHealthTracker = StreamHealthTracker(
        endpoint = endpoint,
        feedLabel = "Alpaca test stream (FAKEPACA)",
        clock = clock,
    )
    ...
    /**
     * Phase 2.h session counter. Every successful `connect()` opens
     * a fresh session id; every WebSocket callback captures the id
     * of the session it belongs to so a late `onClosed`/`onFailure`
     * from a stopped session cannot mutate a newer session's state.
     */
    ...
    /**
     * Id of the most recent session the *user* asked to close via
     * `disconnect()`. Callbacks that arrive on that session after
     * the user-initiated close are mapped to clean `DISCONNECTED`,
     * not `ERROR`.
     */
    @Volatile
        ...
        lifecycleMutex.withLock {
            ...
            if (credentials == null) {
                ...
                healthTracker.onError(
                    ...
                    message = "No Alpaca credentials configured for the test stream.",
                )
                _connectionStatus.value = MarketDataConnectionStatus.error(
                    source = source,
                    error = MarketDataError.AuthenticationFailed(
                        "No Alpaca credentials configured for the test stream.",
                    ),
                    now = clock(),
                )
                return
    ...
    override suspend fun disconnect() {
        lifecycleMutex.withLock {
            // Mark the active session as intentionally closing so a
            // late OkHttp `onFailure`/`onClosed` cannot demote our
            // DISCONNECTED back to ERROR.
    ...
    internal fun handleStreamMessage(message: AlpacaStreamMessage) {
        ...
        when (message) {
            ...
            is AlpacaStreamMessage.Quote -> Unit  // quotes ignored by the offline pipeline today
    ...
    private fun onStreamError(error: AlpacaStreamMessage.StreamError) {
        val mapped = when (error.code) {
            ...
            405, 410 -> MarketDataError.SubscriptionRejected(error.message, symbol = "")
    ...
    private inner class ListenerBridge(
        ...
        override fun onOpen() {
            // Wait for the server's "connected" message before sending auth.
        }
        ...
        override fun onFailure(throwable: Throwable?, response: String?) {
            if (isStale()) return
            if (isIntentionalClose()) {
                // The user just asked to stop. OkHttp fires onFailure
                // after we cancel — treat it as a clean DISCONNECTED.
                handle = null
                currentCredentials = null
                _connectionStatus.value = MarketDataConnectionStatus.disconnected(source, clock())
                healthTracker.onUserStop()
